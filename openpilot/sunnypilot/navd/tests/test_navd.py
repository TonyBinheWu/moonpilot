...
def test_route_message_point_limit_preserves_endpoints():
...
def test_failed_refresh_retains_loaded_route():
...
def test_offline_reroute_does_not_discard_loaded_route_or_call_mapbox():
  session = FailingSession()
  engine = make_engine(session)
  destination = Coordinate(22.3010, 114.1010)
  engine.params.values["NavDestination"] = destination.as_dict()
...
def test_network_gate_accepts_only_wifi_and_cellular():
...
def test_route_response_rejects_empty_data_without_overwriting_route():
  engine = make_engine()
  assert engine._load_route_response(route_response())
  original_route = engine.route
...
def test_loaded_route_produces_local_turn_instruction():
  pm = FakePM()
...
def test_missing_typical_duration_falls_back_to_route_duration():
...
def test_cached_route_restores_for_same_destination():
  params = FakeParams()
  destination = Coordinate(22.3010, 114.1010)
