...
SUPPORTED_LOG = "tici/tizi/mici with valid pose, calibration, external GPS and raw UBlox are required"
CAMERA_STREAMS = {
...
def _frame_info(streams, device_type: int, video_dir: Path) -> dict[str, np.ndarray]:
  ...
  for camera, stream in CAMERA_STREAMS.items():
    ...
    if (
      not indexes
      or any(not event.valid or str(index.type) != "fullHEVC" or not index.timestampSof or index.timestampEof <= index.timestampSof or not index.len
    ...
    frame_info.update({
      ...
      f"{camera}/t": np.asarray([(index.timestampSof + index.timestampEof) * 0.5e-9 for index in indexes]),
...
def read_log(messages, video_dir: Path) -> tuple[LocalizationInput, dict[str, np.ndarray]]:
  ...
  for event in messages:
    ...
    wrong_device = (
      ...
      and str(event.initData.deviceType) not in {"tici", "tizi", "mici"}
    )
  ...
  if len(device_types) != 1 or not device_types <= {"tici", "tizi", "mici"} or not gps_events:
    raise ValueError(SUPPORTED_LOG)
  ...
  for event in streams[POSE_STREAM]:
    ...
    if all(getattr(pose, name).valid for name in ("orientationNED", "velocityDevice", "accelerationDevice", "angularVelocityDevice")):
