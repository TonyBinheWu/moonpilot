...
def _load_images(
    ...
    camera_key = CAMERA_BY_DEVICE_TYPE[device_type]
    if "sensor_name" in frame_info:
        camera_key = (camera_key[0], bytes(frame_info["sensor_name"]).decode("ascii"))
    cameras = DEVICE_CAMERAS[camera_key]
...
def _load_segment(
    ...
    candidates = candidates[localizer["frame_t"][candidates] + T_IDXS[-1] <= localizer["frame_t"][-1]]
    if not len(candidates):
        raise ValueError(f"{segment_dir}: no complete history/future window")
